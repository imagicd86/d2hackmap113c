...
int d2hackmap_id=0;
int d2win_drawMainMenu,d2client_14F0;
...
int Install() {
  ...
  HMODULE addr_d2win=get_dll_addr("D2WIN.DLL");if (!addr_d2win) return 0;
	...
	d2win_drawMainMenu=DLL_ADDR(d2win,0x6F8F87E0);
	d2client_14F0=DLL_ADDR(d2client,0x6FB714F0);
	parseCmdLine();
...
BOOL APIENTRY DllMain(HMODULE hModule,DWORD ul_reason_for_call,LPVOID lpReserved) {
	...
	case DLL_PROCESS_ATTACH:
		...
		*(strrchr(szPluginPath,'\\')+1)=0;
		sprintf(d2hackmap_path0,"%sruntime\\d2hackmap.dll",szPluginPath);
		return Install();
