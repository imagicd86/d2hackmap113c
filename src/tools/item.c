...
void make_charm() {
	int size;
	char *path="t/charm.d2i";
	char *outpath="t/t2.d2i";
	char *b=loadbitfile(path,&size);
	int pos=182;
...
void pcode(char *b,int pos) {
	int code=rbits(b,pos,30);char *s=(char *)&code;
	printf(" type %c%c%c\n",s[0],s[1],s[2]);
}
void show_item() {
	int size,size2;
	char *path,*path2;
	path="t/items/Infinity.d2i";
	path="t/items/Enigma.d2i";
	path="t/items/Thresher(4s).d2i";
	path="t/items/add3fire.d2i";
	//10 04 a0 00 - 65 08 1c 20  67 c7 06 02 
	//10 04 a0 00 - 65 08 1a 20  67 c7 06 02
	char *b=loadbitfile(path,&size);
	//pbits(b,size);
	pcode(b,76);
	char *p=b;
	//search(b,size,11,10);
	if (0) {
		char *path2="t/t/t.d2i";
		//wbits(b,158,11,1000);//item level
		char *code2="paf ";
		wbits(b,76,30,*(int *)code2);//code
		savebits(path2,b,size);
		return;
	}
	int magic=rbits(p,0,16);assert(magic==0x4d4a);
	int socket=rbits(p,27,1);
	int compact=rbits(p,37,1);
	int runeword=rbits(p,42,1);
	if (socket) printf(" socket");
	if (compact) printf(" compact");
	if (runeword) printf(" runeword");
	if (0) {
		char *s4[]={
		"7h7",
		0};
		for (int i=0;s4[i];i++) {
			char path2[256];
			char name[5];name[0]=s4[i][0];name[1]=s4[i][1];name[2]=s4[i][2];name[3]=' ';name[4]=0;
			wbits(p,76,30,*(int *)name);
			wbits(b,143,7,81);//item level
			name[3]=0;
			snprintf(path2,256,"D:/game/items/t/%s.d2i",name);;
			printf("%s\n",path2);
			savebits(path2,b,size);
		}
		return;
	}
	int column=rbits(p,65,4);
	int row=rbits(p,69,3);
	int stash=rbits(p,73,3);
	printf(" (%d:%d,%d)",stash,row,column);
	if (compact) return;
	int nsocketO=rbits(p,108,3);
	printf(" socketOccupied=%d",nsocketO);
	int quality=rbits(p,111,4);
	printf(" quality=%d",quality);
	if (quality>0&&quality<10) printf(" %s",qualityStr[quality]);
	int customGraphic=rbits(p,115,1);
	int ilvl=rbits(p,143,7);printf(" ilvl=%d",ilvl);
	int pos=116;
	if (customGraphic) pos+=3;
	int classSpecific=rbits(p,pos,1);pos++;
	if (classSpecific) pos+=11;
	printf(" POS=%d\n",pos);//162
	if (quality==6) {
		int prefix=rbits(p,pos,8);pos+=8;
		int suffix=rbits(p,pos,8);pos+=8;
		printf("%d %d\n",prefix,suffix);
		for (int i=0;i<3;i++) {
			int mp=rbits(p,pos,1);pos++;
			if (mp) {
				int mpI=rbits(p,pos,11);pos+=11;
			}
			int ms=rbits(p,pos,1);pos++;
			if (ms) {
				int msI=rbits(p,pos,11);pos+=11;
			}
		}
	}
	int realm=rbits(p,pos,1);pos++;
	//assert(realm==0);
	printf(" POS=%d\n",pos);
	int Nstack=rbits(p,pos,9);pos+=9;
	printf(" n=%d",Nstack);
	//174 socket count
	printf("\n");
}
